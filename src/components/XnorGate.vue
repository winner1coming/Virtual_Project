<template>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600" width="300" height="300" style="overflow: visible;">
      <g :transform="`translate(${xnorGate.x}, ${xnorGate.y}) scale(${xnorGate.scale})`" cursor="move">
        <!-- XOR 门图形 -->
        ...
        <!-- 输出状态 -->
        <OutputPort :cx="497" :cy="288" :active="xnorGate.output" />
      </g>
    </svg>
  </template>
  ...
  let inputYs = useGateLayout(xnorGate.inputCount)
  ...
  function handleToggleInput(index) {
    toggleInput(xnorGate, index, updateOutput)
    // setInputCount(2)
  ...
  function handleSetInputInverted(index, inverted) {
    setInputInverted(xnorGate, index, inverted, updateOutput)
  }
  
  function handleSetScale(newscale)
  {
    setScale(xnorGate, newscale)
  }
  
  function updateOutput() {
    xnorGate.output = !xnorGate.inputs.reduce((acc, input) => {
  ...
  {
    xnorGate.inputCount = newCount;
    inputYs = useGateLayout(xnorGate.inputCount)
  ...
  onMounted(()=>{
   xnorGate.inputs = computed(()=>createInputs(xnorGate.inputCount));
   updateOutput();
  })
