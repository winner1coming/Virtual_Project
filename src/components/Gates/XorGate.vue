<template>
  <g :transform="`translate(${xorGate.offset[0]*xorGate.scale}, ${xorGate.offset[1]*xorGate.scale}) scale(${xorGate.scale})`" cursor="move">
    <!-- XOR 门图形 -->
    ...
    <!-- 输出状态 -->
    <OutputPort :cx="497" :cy="288" :active="xorGate.outputs[0]" />
  </g>
</template>
...
const xorGate = computed(() => {
  return circuitStore.getComponent(props.id);  
});

const bezierYMin = 179.5;
const bezierYMax = 397;
let minY = computed(()=>Math.min(...xorGate.value.inputPinPosition.map(pin => pin[1])));
let maxY = computed(()=>Math.max(...xorGate.value.inputPinPosition.map(pin => pin[1])));
